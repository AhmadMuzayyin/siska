                ...
                <div class="container mx-auto px-4 sm:px-6 max-w-4xl">
                    <div class="rounded-3xl border border-[#d6eda6] bg-white p-8 sm:p-12 shadow-xl shadow-lime-900/5">
                        ...
                        @if ($submitted || session('status'))
                            <div class="mb-8 rounded-3xl bg-[#f0f8ec] border-2 border-[#6bb82d] p-6 sm:p-8 text-[#2e5b18] flex flex-col sm:flex-row items-center gap-5 shadow-sm text-center sm:text-left">
                                <div class="size-16 rounded-2xl bg-[#6bb82d] text-white flex items-center justify-center shrink-0 shadow-md">
                                    <flux:icon name="check-circle" class="size-9" />
                                </div>
                                <div class="space-y-1">
                                    <h4 class="font-black text-lg sm:text-xl text-[#2e5b18]">{{ __('Alhamdulillah, Formulir Berhasil Dikirim!') }}</h4>
                                    <p class="text-xs sm:text-sm text-zinc-700 leading-relaxed">{{ session('status') ?? __('Data calon santri telah masuk ke sistem dan akan segera ditinjau oleh panitia penerimaan santri baru.') }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        <form wire:submit="register" class="flex flex-col gap-10">
                            ...
                            <div class="flex justify-end pt-6 border-t border-[#d6eda6]">
                                <button 
                                    ...
                                    class="inline-flex items-center gap-2.5 rounded-full bg-[#6bb82d] hover:bg-[#5ca828] text-white font-extrabold text-sm px-10 py-4 shadow-xl shadow-lime-600/30 transition-all duration-300 transform hover:-translate-y-0.5 cursor-pointer disabled:opacity-60"
                                >
                                    <flux:icon name="user-plus" class="size-4" wire:loading.remove wire:target="register" />
                                    <flux:icon name="arrow-path" class="size-4 animate-spin" wire:loading wire:target="register" />
                                    <span wire:loading.remove wire:target="register">{{ __('Kirim Formulir Pendaftaran') }}</span>
                                    <span wire:loading wire:target="register">{{ __('Mengirim Data...') }}</span>
                                </button>
                            </div>
            ...
            <section class="w-full py-16">
                <div class="mx-auto max-w-4xl px-6">
                    <div class="rounded-3xl border border-emerald-600/30 bg-white p-8 sm:p-12 shadow-2xl shadow-emerald-950/10">
                        ...
                        @if ($submitted || session('status'))
                            <div class="mb-8 rounded-3xl bg-emerald-50 border-2 border-emerald-500 p-6 sm:p-8 text-emerald-900 flex flex-col sm:flex-row items-center gap-5 shadow-sm text-center sm:text-left">
                                <div class="size-16 rounded-2xl bg-emerald-600 text-white flex items-center justify-center shrink-0 shadow-md">
                                    <flux:icon name="check-circle" class="size-9" />
                                </div>
                                <div class="space-y-1">
                                    <h4 class="font-black text-lg sm:text-xl text-emerald-950">{{ __('Alhamdulillah, Pendaftaran Berhasil Dikirim!') }}</h4>
                                    <p class="text-xs sm:text-sm text-zinc-700 leading-relaxed">{{ session('status') ?? __('Data calon santri telah tersimpan dan menunggu konfirmasi admin lembaga.') }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        <form wire:submit="register" class="flex flex-col gap-10">
                            ...
                            <div class="flex justify-end pt-6 border-t border-emerald-100">
                                <button 
                                    type="submit" 
                                    wire:loading.attr="disabled"
                                    class="inline-flex items-center gap-2.5 rounded-2xl bg-emerald-700 hover:bg-emerald-800 text-white font-extrabold text-sm px-10 py-4 shadow-xl shadow-emerald-800/25 transition-all duration-300 transform hover:-translate-y-0.5 cursor-pointer disabled:opacity-60"
                                >
                                    <flux:icon name="user-plus" class="size-4" wire:loading.remove wire:target="register" />
                                    <flux:icon name="arrow-path" class="size-4 animate-spin" wire:loading wire:target="register" />
                                    <span wire:loading.remove wire:target="register">{{ __('Kirim Pendaftaran Santri Baru') }}</span>
                                    <span wire:loading wire:target="register">{{ __('Mengirim Data...') }}</span>
                                </button>
                            </div>
